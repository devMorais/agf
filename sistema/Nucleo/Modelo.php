...
abstract class Modelo
{
    ...
    protected function cadastrar(array $dados): ?int
    {
        try {
            ...
            return null; // Adicionado para retornar null em caso de exceção
        }
    }
     ...
     * @return int
     */
    protected function atualizar(array $dados, string $termos): bool
    {
        try {
            $set = [];
            ...
            return true; // Alterado para retornar true se a execução for bem-sucedida
        } catch (\PDOException $ex) {
            $this->erro = $ex->getMessage();
            return false; // Adicionado para retornar false em caso de exceção
        }
    }
    ...
    {
        $dados = $this->armazenar();

        // CADASTRAR
        if (empty($this->id)) {
            $id = $this->cadastrar($dados);
            if ($id) {
                $this->dados = $this->buscaPorId($id)->dados();
                return true;
            } else {
                $this->mensagem->erro('Erro de sistema: ' . $this->erro);
                return false;
            }
        }
